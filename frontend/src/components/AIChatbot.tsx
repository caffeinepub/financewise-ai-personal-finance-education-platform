...
import { MessageCircle, X, Send, Minimize2, Maximize2, Loader2, AlertCircle, TrendingUp, Lightbulb, Brain, GraduationCap, PiggyBank } from 'lucide-react';
...
export default function AIChatbot() {
  ...
  const { data: balanceData = 0 } = useGetBalance();
          ...
          <div className="flex-1 flex items-center justify-center p-4 overflow-y-auto">
            <div className="text-center space-y-4 max-w-sm">
              ...
              <div className="grid grid-cols-2 gap-2 pt-4">
                <Button variant="outline" size="sm" onClick={() => setInputMessage('How can I start budgeting?')} className="text-xs h-auto py-2">
                  <Lightbulb className="h-3 w-3 mr-1 shrink-0" />
                  <span className="truncate">Budgeting tips</span>
                </Button>
                <Button variant="outline" size="sm" onClick={() => setInputMessage('What is SIP investment?')} className="text-xs h-auto py-2">
                  <TrendingUp className="h-3 w-3 mr-1 shrink-0" />
                  <span className="truncate">Learn SIP</span>
                </Button>
                <Button variant="outline" size="sm" onClick={() => setInputMessage('How to save money?')} className="text-xs h-auto py-2">
                  <PiggyBank className="h-3 w-3 mr-1 shrink-0" />
                  <span className="truncate">Saving strategies</span>
                </Button>
                <Button variant="outline" size="sm" onClick={() => setInputMessage('Explain stocks')} className="text-xs h-auto py-2">
