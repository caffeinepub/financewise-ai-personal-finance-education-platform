...
import { BudgetModeKey, BUDGET_MODE_ALLOCATIONS } from '../lib/budget-planner/budgetModes';
...
export default function BudgetAllocationChart({
  ...
  const total = actual.needsPercent + actual.wantsPercent + actual.savingsPercent;
    ...
    <div className="space-y-4">
      <div>
        <div className="flex justify-between text-xs text-muted-foreground mb-1">
          ...
          <span className="text-xs">{needsPct}% / {wantsPct}% / {savingsPct}%</span>
        </div>
      ...
      <div>
        <div className="flex justify-between text-xs text-muted-foreground mb-1">
          <span>Target ({BUDGET_MODE_ALLOCATIONS[mode].needs}% / {BUDGET_MODE_ALLOCATIONS[mode].wants}% / {BUDGET_MODE_ALLOCATIONS[mode].savings}%)</span>
        </div>
