import { SmartBudgetCategory } from './budgetModes';
import { BudgetModeKey, BUDGET_MODE_ALLOCATIONS } from './budgetModes';
...
export function calculateBudgetTotals(categories: SmartBudgetCategory[]): BudgetTotals {
  let needsTotal = 0;
  let wantsTotal = 0;
  let savingsTotal = 0;
  ...
  return { needsTotal, wantsTotal, savingsTotal, grandTotal: needsTotal + wantsTotal + savingsTotal };
}
...
export function calculateBudgetPercentages(
  ...
  if (monthlyIncome <= 0) return { needsPercent: 0, wantsPercent: 0, savingsPercent: 0 };
  return {
