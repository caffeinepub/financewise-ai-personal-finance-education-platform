import { SmartBudgetCategory, BudgetModeKey, BUDGET_MODE_ALLOCATIONS } from './budgetModes';
import { calculateBudgetTotals, calculateBudgetPercentages } from './budgetCalculations';
...
export function generateBudgetSuggestions(
  ...
  const emergencyCat = categories.find(c => c.name.toLowerCase().includes('emergency'));
  if (!emergencyCat || emergencyCat.amount === 0) {
    suggestions.push({
      id: 'emergency-1',
      type: 'tip',
      message: 'No emergency fund allocation found. Aim to save 3–6 months of expenses for financial security.',
  ...
  if (suggestions.length < 3) {
    suggestions.push({
      id: 'general-1',
      type: 'opportunity',
      message: 'Review your budget monthly and adjust categories as your income or expenses change.',
    });
  }
