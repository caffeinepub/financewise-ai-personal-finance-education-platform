...
import { generateResponse, type AssistantContext, type AssistantResponse } from '../lib/deterministicAssistant';
...
export function useUpdateTransaction() {
  ...
  return useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<TransactionData> }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useUpdateSavingsGoal() {
  ...
  return useMutation({
    mutationFn: async ({ goalId, updatedGoal }: { goalId: string; updatedGoal: SavingsGoal }) => {
      if (!actor) throw new Error('Actor not available');
...
export function useAddChatMessage() {
  ...
  return useMutation({
    mutationFn: async ({ sessionId, message }: { sessionId: string; message: ChatMessage }) => {
      if (!actor) throw new Error('Actor not available');
...
const mockBlogPosts = [
  { id: '1', title: 'Master Your Money: The Complete Guide to Personal Finance Fundamentals', slug: 'personal-finance-fundamentals-guide', excerpt: 'Learn the essential building blocks of personal finance.', featuredImage: '/assets/generated/blog-personal-finance.dim_800x600.jpg', publicationDate: Date.now() - 86400000 * 5, seoMeta: '' },
  { id: '2', title: 'Smart Saving Strategies: How to Build Wealth on Any Income', slug: 'smart-saving-strategies-build-wealth', excerpt: 'Discover proven saving strategies.', featuredImage: '/assets/generated/blog-saving-investing.dim_800x600.jpg', publicationDate: Date.now() - 86400000 * 10, seoMeta: '' },
  { id: '3', title: 'Budgeting Methods That Actually Work: 50/30/20, Zero-Based & More', slug: 'budgeting-methods-that-work', excerpt: 'Explore practical budgeting methods.', featuredImage: '/assets/generated/blog-budgeting-system.dim_800x600.jpg', publicationDate: Date.now() - 86400000 * 15, seoMeta: '' },
  { id: '4', title: 'Investing for Beginners: Your First Steps into the Stock Market', slug: 'investing-for-beginners-first-steps', excerpt: 'Demystify investing with this beginner-friendly guide.', featuredImage: '/assets/generated/blog-investment-basics.dim_800x600.jpg', publicationDate: Date.now() - 86400000 * 20, seoMeta: '' },
  { id: '5', title: 'Stock Market Basics: Understanding How Markets Work', slug: 'stock-market-basics-understanding-markets', excerpt: 'Get a clear understanding of how stock markets function.', featuredImage: '/assets/generated/blog-stock-market-basics.dim_800x600.jpg', publicationDate: Date.now() - 86400000 * 25, seoMeta: '' },
  { id: '6', title: 'Mutual Funds Explained: A Complete Guide for New Investors', slug: 'mutual-funds-complete-guide', excerpt: 'Understand mutual funds inside and out.', featuredImage: '/assets/generated/blog-mutual-funds-explained.dim_800x600.jpg', publicationDate: Date.now() - 86400000 * 30, seoMeta: '' },
  { id: '7', title: 'SIP Investment Strategy: Build Wealth Through Systematic Investing', slug: 'sip-investment-strategy-systematic-investing', excerpt: 'Learn how SIPs help you build wealth consistently.', featuredImage: '/assets/generated/blog-sip-investment-guide.dim_800x600.jpg', publicationDate: Date.now() - 86400000 * 35, seoMeta: '' },
  { id: '8', title: 'Emergency Fund Essentials: Why You Need One and How to Build It', slug: 'emergency-fund-essentials-how-to-build', excerpt: 'Discover why an emergency fund is your financial safety net.', featuredImage: '/assets/generated/blog-emergency-fund.dim_800x600.jpg', publicationDate: Date.now() - 86400000 * 40, seoMeta: '' },
  { id: '9', title: 'Debt Management: Proven Strategies to Become Debt-Free Faster', slug: 'debt-management-strategies-debt-free', excerpt: 'Take control of your debt with proven strategies.', featuredImage: '/assets/generated/blog-debt-management.dim_800x600.jpg', publicationDate: Date.now() - 86400000 * 45, seoMeta: '' },
  { id: '10', title: 'Financial Goal Setting: Turn Dreams into Achievable Milestones', slug: 'financial-goal-setting-achievable-milestones', excerpt: 'Transform vague financial wishes into concrete goals.', featuredImage: '/assets/generated/blog-financial-goals.dim_800x600.jpg', publicationDate: Date.now() - 86400000 * 50, seoMeta: '' },
];
...
export function useProcessChatMessage() {
  ...
  return useMutation({
    mutationFn: async (userMessage: string): Promise<AssistantResponse> => {
      ...
      const totalIncome = txList.filter((t) => t.transactionType === 'income').reduce((s, t) => s + t.amount, 0);
      const totalExpenses = txList.filter((t) => t.transactionType === 'expense').reduce((s, t) => s + t.amount, 0);
