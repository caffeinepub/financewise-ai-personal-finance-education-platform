...
import { useGetUserTransactions, useAddTransaction } from '../hooks/useQueries';
...
import { TrendingUp, TrendingDown, PlusCircle, Loader2, BarChart2, PieChartIcon, Activity } from 'lucide-react';
...
export default function Analytics() {
  ...
  const expensePieData = useMemo(() => {
    ...
    transactions.forEach((t) => {
      if (t.transactionType === 'expense') map.set(t.category, (map.get(t.category) || 0) + Number(t.amount));
    });
    return Array.from(map.entries()).map(([name, value]) => ({
      name, value, fill: EXPENSE_COLORS[name] || '#8884d8',
    }));
  }, [transactions]);
  ...
  const expenseBarData = useMemo(() => {
    ...
    return Array.from(map.entries())
      .map(([category, amount]) => ({ category, amount, fill: EXPENSE_COLORS[category] || '#8884d8' }))
  ...
  const expenseLineData = useMemo(() => {
    ...
    transactions.forEach((t) => {
      if (t.transactionType === 'expense') {
        const d = new Date(Number(t.date) / 1_000_000).toISOString().split('T')[0];
  ...
  const incomePieData = useMemo(() => {
    const map = new Map<string, number>();
    transactions.forEach((t) => {
      if (t.transactionType === 'income') map.set(t.category, (map.get(t.category) || 0) + Number(t.amount));
    });
    return Array.from(map.entries()).map(([name, value]) => ({
      name, value, fill: INCOME_COLORS[name] || '#10b981',
    }));
  }, [transactions]);
  ...
  const incomeBarData = useMemo(() => {
    ...
    return Array.from(map.entries())
      .map(([category, amount]) => ({ category, amount, fill: INCOME_COLORS[category] || '#10b981' }))
      .sort((a, b) => b.amount - a.amount);
  }, [transactions]);
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setExpensePaymentType(''); setExpenseDate(new Date().toISOString().split('T')[0]);
  ...
  const handleAddIncome = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIncomeAmount(''); setIncomeCategory(''); setIncomeNote('');
      setIncomePaymentType(''); setIncomeDate(new Date().toISOString().split('T')[0]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 p-4 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Visualize your financial data with detailed charts</p>
        </div>

        {/* Sliding Tabs */}
        <Tabs defaultValue="expenses" className="w-full">
          <TabsList className="w-full max-w-sm mb-6 h-11">
            <TabsTrigger value="expenses" className="flex-1 gap-2 text-sm font-semibold">
              <TrendingDown className="h-4 w-4 text-red-500" />
              Expenses Analytics
            </TabsTrigger>
            <TabsTrigger value="income" className="flex-1 gap-2 text-sm font-semibold">
          ...
          <TabsContent value="expenses" className="space-y-6 mt-0">
            ...
            <Card>
              ...
              <CardContent>
                <form onSubmit={handleAddExpense} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <div className="space-y-2">
                    <Label htmlFor="expense-amount">Amount ({symbol}) *</Label>
                    <div className="relative flex items-center">
                      <span className="absolute left-3 text-muted-foreground text-sm pointer-events-none">{symbol}</span>
                      <Input
                        id="expense-amount"
                        type="number"
                        step="0.01"
                  ...
                  <div className="space-y-2">
                    <Label>Date</Label>
                    <Input type="date" value={expenseDate} onChange={(e) => setExpenseDate(e.target.value)} disabled={addTransaction.isPending} />
                  </div>
                  <div className="space-y-2 sm:col-span-2 lg:col-span-1">
                    <Label>Note (Optional)</Label>
                    <Textarea placeholder="Add a note..." value={expenseNote} onChange={(e) => setExpenseNote(e.target.value)} disabled={addTransaction.isPending} className="resize-none h-10" />
                  </div>
                  <div className="flex items-end sm:col-span-2 lg:col-span-3">
                    <Button type="submit" className="w-full sm:w-auto" disabled={addTransaction.isPending}>
              ...
              <Card>
                ...
                <CardContent>
                  {expenseBarData.length > 0 ? (
                    <ResponsiveContainer width="100%" height={260}>
                      <BarChart data={expenseBarData} layout="vertical" margin={{ left: 8, right: 16 }}>
                        <CartesianGrid strokeDasharray="3 3" horizontal={false} />
                        <XAxis type="number" tickFormatter={(v) => format(v)} tick={{ fontSize: 11 }} />
                        <YAxis type="category" dataKey="category" tick={{ fontSize: 11 }} width={110} />
                        <Tooltip formatter={(v: number) => format(v)} />
                        <Bar dataKey="amount" radius={[0, 4, 4, 0]}>
                          {expenseBarData.map((entry, i) => <Cell key={i} fill={entry.fill} />)}
                        </Bar>
                      </BarChart>
                    </ResponsiveContainer>
                  ) : <EmptyChartState label="expense" />}
                ...
                {expenseLineData.some((d) => d.amount > 0) ? (
                  <ResponsiveContainer width="100%" height={260}>
                    <LineChart data={expenseLineData}>
                      ...
                      <YAxis tickFormatter={(v) => format(v)} tick={{ fontSize: 11 }} />
                      <Tooltip formatter={(v: number) => format(v)} labelFormatter={(l) => `Day ${l}`} />
                      <Line type="monotone" dataKey="amount" stroke="#ef4444" name="Expenses" dot={false} strokeWidth={2} />
                    </LineChart>
                  </ResponsiveContainer>
          ...
          <TabsContent value="income" className="space-y-6 mt-0">
            ...
            <Card>
              ...
              <CardContent>
                <form onSubmit={handleAddIncome} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <Label>Date</Label>
                    <Input type="date" value={incomeDate} onChange={(e) => setIncomeDate(e.target.value)} disabled={addTransaction.isPending} />
                  </div>
                  <div className="space-y-2 sm:col-span-2 lg:col-span-1">
                    <Label>Note (Optional)</Label>
                    <Textarea placeholder="Add a note..." value={incomeNote} onChange={(e) => setIncomeNote(e.target.value)} disabled={addTransaction.isPending} className="resize-none h-10" />
                  </div>
                  <div className="flex items-end sm:col-span-2 lg:col-span-3">
                    <Button type="submit" className="w-full sm:w-auto" disabled={addTransaction.isPending}>
                      {addTransaction.isPending
              ...
              <Card>
                ...
                <CardContent>
                  {incomeBarData.length > 0 ? (
                    <ResponsiveContainer width="100%" height={260}>
                      <BarChart data={incomeBarData} layout="vertical" margin={{ left: 8, right: 16 }}>
                        <CartesianGrid strokeDasharray="3 3" horizontal={false} />
                        <XAxis type="number" tickFormatter={(v) => format(v)} tick={{ fontSize: 11 }} />
                ...
                {incomeLineData.some((d) => d.amount > 0) ? (
                  <ResponsiveContainer width="100%" height={260}>
                    <LineChart data={incomeLineData}>
                      ...
                      <YAxis tickFormatter={(v) => format(v)} tick={{ fontSize: 11 }} />
                      <Tooltip formatter={(v: number) => format(v)} labelFormatter={(l) => `Day ${l}`} />
                      <Line type="monotone" dataKey="amount" stroke="#10b981" name="Income" dot={false} strokeWidth={2} />
                    </LineChart>
                  </ResponsiveContainer>
