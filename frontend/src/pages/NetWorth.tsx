...
import type { Asset, Liability, AssetCategory, LiabilityCategory } from '../backend';
import { AssetCategory as AssetCategoryEnum, LiabilityCategory as LiabilityCategoryEnum } from '../backend';
...
import {
  TrendingUp, TrendingDown, Plus, Trash2, Wallet, Building2,
  PiggyBank, BarChart3, Coins, Home, Gem, CreditCard, AlertCircle, Loader2
} from 'lucide-react';
...
function getAssetIcon(category: AssetCategory) {
  switch (category) {
    case AssetCategoryEnum.cash: return <Wallet className="w-4 h-4" />;
    case AssetCategoryEnum.bankBalance: return <Building2 className="w-4 h-4" />;
    case AssetCategoryEnum.savingsAccount: return <PiggyBank className="w-4 h-4" />;
    case AssetCategoryEnum.investments: return <BarChart3 className="w-4 h-4" />;
    case AssetCategoryEnum.stocks: return <TrendingUp className="w-4 h-4" />;
    case AssetCategoryEnum.mutualFunds: return <BarChart3 className="w-4 h-4" />;
    case AssetCategoryEnum.gold: return <Gem className="w-4 h-4" />;
    case AssetCategoryEnum.property: return <Home className="w-4 h-4" />;
...
function generateNetWorthInsights(
  ...
  if (netWorth < 0) {
    insights.push('Your liabilities exceed your assets. Focus on paying down high-interest debt first.');
    insights.push('Consider creating a debt repayment plan — target the highest-interest debt first (avalanche method).');
  } else if (netWorth < totalAssets * 0.2) {
    insights.push(`Your debt-to-asset ratio is ${Math.round(debtRatio)}%. Aim to reduce it below 30% for financial stability.`);
    insights.push('Reduce credit card usage and avoid taking on new debt until existing liabilities are under control.');
  } else {
    insights.push(`Great job! Your net worth is ${format(netWorth)}. Keep building assets consistently.`);
  }

  if (totalAssets > 0) {
    insights.push(`Increase savings by investing regularly — even small monthly contributions compound significantly over time.`);
  } else {
    insights.push('Start by adding your assets — cash, bank balance, and savings accounts — to get a clear financial picture.');
  }

  insights.push('Build an emergency fund of 6 months of expenses before making large investments.');
  insights.push('Diversify your assets across cash, investments, and property to reduce financial risk.');
...
export default function NetWorth() {
  ...
  const [newAssetCategory, setNewAssetCategory] = useState<AssetCategory>(AssetCategoryEnum.cash);
  const [newAssetAmount, setNewAssetAmount] = useState('');
  const [newLiabilityName, setNewLiabilityName] = useState('');
  const [newLiabilityCategory, setNewLiabilityCategory] = useState<LiabilityCategory>(LiabilityCategoryEnum.loans);
  ...
  const assetProgress = totalAssets > 0 ? Math.min(100, (totalAssets / (totalAssets + totalLiabilities)) * 100) : 0;

  const insights = generateNetWorthInsights(totalAssets, totalLiabilities, netWorth, format);
  ...
  const handleAddLiability = async () => {
    if (!newLiabilityName.trim() || !newLiabilityAmount || !actor || !identity) return;
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="border-green-500/30 bg-green-500/5">
          <CardContent className="pt-4 pb-4">
            ...
            <p className="text-2xl font-bold text-green-500 mt-1">{format(totalAssets)}</p>
        ...
        <Card className="border-red-500/30 bg-red-500/5">
          <CardContent className="pt-4 pb-4">
            ...
            <p className="text-2xl font-bold text-red-500 mt-1">{format(totalLiabilities)}</p>
            <p className="text-xs text-muted-foreground mt-1">{liabilities.length} items</p>
          </CardContent>
        </Card>
        <Card className={`${isPositive ? 'border-primary/30 bg-primary/5' : 'border-destructive/30 bg-destructive/5'}`}>
          <CardContent className="pt-4 pb-4">
            <p className="text-xs text-muted-foreground uppercase tracking-wide">Net Worth</p>
            <p className={`text-2xl font-bold mt-1 ${isPositive ? 'text-primary' : 'text-destructive'}`}>
              {format(netWorth)}
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              {isPositive ? '✓ Positive position' : '⚠ Liabilities exceed assets'}
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card>
          <CardContent className="pt-4 pb-4">
            <div className="flex justify-between text-sm mb-2">
              ...
              <span className="font-medium">{Math.round(assetProgress)}% assets</span>
            </div>
            <Progress value={assetProgress} className="h-3" />
            <div className="flex justify-between text-xs text-muted-foreground mt-1">
              <span className="text-green-500">Assets: {format(totalAssets)}</span>
              <span className="text-red-500">Liabilities: {format(totalLiabilities)}</span>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <Card className={`${isPositive ? 'border-green-500/30' : 'border-amber-500/30'}`}>
        <CardContent className="pt-4 pb-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Net Worth = {format(totalAssets)} (Assets) − {format(totalLiabilities)} (Liabilities) = {format(netWorth)}
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" onClick={() => setShowAddAsset(true)} className="h-8 gap-1">
              ...
              assets.map((asset, index) => (
                <div key={index} className="flex items-center justify-between p-2 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                  <div className="flex items-center gap-2">
                    <span className="text-green-500">{getAssetIcon(asset.category)}</span>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-green-500">{format(asset.amount)}</span>
                    <Button
                      variant="ghost"
                      size="icon"
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" onClick={() => setShowAddLiability(true)} className="h-8 gap-1">
                <Plus className="w-3 h-3" /> Add
              </Button>
            </div>
          </CardHeader>
                ...
                <div key={index} className="flex items-center justify-between p-2 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {LIABILITY_CATEGORIES.find(c => c.value === liability.category)?.label}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-red-500">{format(liability.amount)}</span>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-7 w-7 text-muted-foreground hover:text-destructive"
            ...
            {liabilities.length > 0 && (
              <>
                ...
                <div className="flex justify-between text-sm font-semibold">
                  ...
                  <span className="text-red-500">{format(totalLiabilities)}</span>
                </div>
              </>
            )}
          </CardContent>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select value={newAssetCategory} onValueChange={(v) => setNewAssetCategory(v as AssetCategory)}>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!newAssetName.trim() || !newAssetAmount || saveNetWorthData.isPending}
            >
              {saveNetWorthData.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
              Add Asset
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showAddLiability} onOpenChange={setShowAddLiability}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label>Category</Label>
              <Select value={newLiabilityCategory} onValueChange={(v) => setNewLiabilityCategory(v as LiabilityCategory)}>
                <SelectTrigger className="mt-1">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!newLiabilityName.trim() || !newLiabilityAmount || saveNetWorthData.isPending}
            >
              {saveNetWorthData.isPending ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
              Add Liability
            </Button>
