...
import { useGetUserTransactions, useGetBalance, useGetSavingsGoals } from '../hooks/useQueries';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Alert, AlertDescription } from '@/components/ui/alert';
import { AlertTriangle, Info, TrendingUp, Zap, Cpu, Brain } from 'lucide-react';
...
export default function AIInsights() {
  ...
  const insights = useMemo(() => {
    const totalIncome = transactions.filter((t) => t.transactionType === 'income').reduce((s, t) => s + Number(t.amount), 0);
    const totalExpenses = transactions.filter((t) => t.transactionType === 'expense').reduce((s, t) => s + Number(t.amount), 0);
    ...
    return { totalIncome, totalExpenses, netSavings, savingsRate, riskLevel, totalGoalTarget, totalGoalCurrent, goalProgress };
  }, [transactions, goals]);
      ...
      <div className="min-h-screen p-4 md:p-6 lg:p-8">
        <Card className="max-w-2xl mx-auto">
          <CardHeader>
            ...
            <CardDescription>Add transactions to unlock AI-powered insights</CardDescription>
          </CardHeader>
          <CardContent>
            <Alert>
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 lg:p-8 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">Advanced machine learning insights powered by 3 ML models</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent><div className="text-2xl font-bold text-green-600">{formatCurrency(insights.totalIncome)}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardDescription>Total Expenses</CardDescription></CardHeader>
          <CardContent><div className="text-2xl font-bold text-red-600">{formatCurrency(insights.totalExpenses)}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardDescription>Net Savings</CardDescription></CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2"><CardDescription>Savings Rate</CardDescription></CardHeader>
          <CardContent><div className="text-2xl font-bold">{insights.savingsRate.toFixed(1)}%</div></CardContent>
        </Card>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={categorySpending} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label={(e) => e.name}>
                  {categorySpending.map((_entry, index) => (
                    <Cell key={`cell-${index}`} fill={CATEGORY_COLORS[_entry.name] || '#8884d8'} />
                  ))}
                </Pie>
