...
export function generateRecommendations(
  ...
  const savingsRate =
    (plan.allocations.savings / plan.allocations.totalIncome) * 100;
  ...
  recommendations.investingGuidance = {
    overview:
      inputs.financialKnowledge === "beginner"
  ...
  recommendations.financialImprovement = {
    overview:
      "Improving your finances is a gradual process. Focus on building good habits, reducing unnecessary expenses, and increasing your financial literacy.",
    tips: [],
  };
...
export function generateActionPlan(inputs: BudgetInputs, plan: any): string[] {
  ...
  const hasHighDiscretionary =
    plan.allocations.wants > plan.allocations.needs * 0.6;
  ...
  if (inputs.loanEMIs > 0) {
    ...
    inputs.emergencyFund < plan.allocations.totalExpenses * 3
  ) {
    actionPlan.push(
