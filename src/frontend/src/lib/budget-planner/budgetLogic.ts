...
export function generateBudgetPlan(inputs: BudgetInputs): BudgetPlan {
  const totalIncome =
    inputs.monthlyIncome + inputs.secondaryIncome + inputs.passiveIncome;

  // Calculate total expenses
  const coreExpenses = inputs.rent + inputs.foodGroceries + inputs.transport;
  const fixedExpenses =
    inputs.utilities +
    inputs.internetMobile +
    inputs.insurance +
    inputs.educationFees +
    inputs.loanEMIs;
  const variableExpenses =
    inputs.shopping + inputs.entertainment + inputs.travel + inputs.medical;
  ...
  if (fixedExpenses > 0) {
    ...
    assumptions.push(
      "Fixed expenses (utilities, internet, insurance) estimated at 10% of income",
    );
  }
  ...
  if (variableExpenses > 0) {
    ...
    assumptions.push(
      "Variable expenses (shopping, entertainment) estimated at 15% of income",
    );
  }
  ...
  if (!inputs.targetAmount && !inputs.timeline) {
    assumptions.push(
      `${rule} budgeting rule applied based on your savings preference`,
    );
  }
  ...
  const savingsProgressPercentage = Math.min(
    (currentSavings / targetSavings) * 100,
    100,
  );
