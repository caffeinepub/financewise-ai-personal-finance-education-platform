...
export function calculateSavingPlan(
  ...
  const monthlyRequired = Math.max(
    remaining / 24,
    monthlySavingsCapacity * 0.2,
  );
  const monthsToGoal =
    remaining <= 0 ? 0 : Math.ceil(remaining / monthlyRequired);
...
export function generateOptimizedPlan(
  ...
  const boostedMonthly = Math.max(monthlySavingsCapacity * 0.3, remaining / 12);
  const monthsToGoal =
    remaining <= 0 ? 0 : Math.ceil(remaining / boostedMonthly);
  return {
    strategy:
      "Increase monthly savings by 30% and automate transfers on payday",
