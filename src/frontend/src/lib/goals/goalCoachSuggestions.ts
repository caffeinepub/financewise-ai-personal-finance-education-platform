...
export function generateExpenseOptimizationSuggestions(
  ...
  topCategories.forEach((cat, index) => {
    ...
    const estimatedSavings = cat.totalAmount * (reductionPercent / 100);

    suggestions.push({
      category: cat.category,
      currentSpend: cat.totalAmount,
      reductionPercent,
      estimatedSavings,
...
export function getIncomeBoostIdeas(): IncomeBoostIdea[] {
  return [
    ...
    },
    {
      ...
      description:
        "Declutter and sell items you no longer need on marketplaces",
    },
    {
      title: "Weekend Part-Time Work",
      description:
        "Take on weekend gigs or part-time roles to supplement income",
    },
    {
