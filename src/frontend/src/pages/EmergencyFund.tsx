...
} from "../lib/transactions/monthlyAggregates";
...
export default function EmergencyFund() {
  ...
  const optimizedPlan = generateOptimizedPlan(
    remaining,
  ...
  const handleSave = async () => {
    if (!identity) return;
    const amount = Number.parseFloat(savedAmountInput);
        ...
        <CardContent className="pt-4 pb-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold text-sm">
              {emergencyLevel === "Safe" &&
                "Your emergency fund is strong! You have 6+ months covered."}
              {emergencyLevel === "Moderate" &&
                `You have ${monthsCovered.toFixed(1)} months covered. Aim for 6 months for full security.`}
              {emergencyLevel === "Low" &&
                `You have ${monthsCovered.toFixed(1)} months covered. Try to build up to at least 3 months as soon as possible.`}
            </p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-lg bg-muted/30">
              ...
              <p className="text-xl font-bold text-primary mt-1">
                {format(savingPlan.monthlyRequired)}
              </p>
            </div>
            <div className="p-3 rounded-lg bg-muted/30">
              <p className="text-xs text-muted-foreground">
              ...
              <p className="text-xl font-bold text-foreground mt-1">
                {remaining <= 0
                  ? "Achieved! 🎉"
                  : `${savingPlan.monthsToGoal} months`}
              </p>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-2">
                <div>
                  ...
                  <p className="text-base font-bold text-primary">
                    {format(optimizedPlan.monthlyRequired)}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-base font-bold text-foreground">
                    {remaining <= 0
                      ? "Done!"
                      : `${optimizedPlan.monthsToGoal} months`}
                  </p>
                </div>
              </div>
            </div>
