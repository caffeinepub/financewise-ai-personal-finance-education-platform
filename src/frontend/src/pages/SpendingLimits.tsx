...
export default function SpendingLimits() {
  ...
  for (const cat of SPENDING_CATEGORIES) {
    let total = 0;
    for (const tc of cat.transactionCategories) {
  ...
  const effectiveLimits: SpendingLimit[] = SPENDING_CATEGORIES.map((cat) => ({
    category: cat.value,
    limitAmount: Number.parseFloat(limitInputs[String(cat.value)] ?? "0") || 0,
  ...
  const handleSaveAll = async () => {
    ...
    const limits: SpendingLimit[] = SPENDING_CATEGORIES.map((cat) => ({
      category: cat.value,
      limitAmount:
        Number.parseFloat(limitInputs[String(cat.value)] ?? "0") || 0,
      user: identity.getPrincipal(),
      lastUpdated: BigInt(Date.now()),
    }));
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 max-w-4xl mx-auto">
      ...
      <Alert
        className={`${
          contextualMessage.includes("Excellent") ||
          contextualMessage.includes("within limits in all")
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {SPENDING_CATEGORIES.map((cat) => {
          ...
          const limit = Number.parseFloat(limitInputs[catKey] ?? "0") || 0;
                  ...
                  <>
                    ...
                    {status === "alert" && (
                      <div className="flex items-center gap-1 text-xs text-red-500 bg-red-500/10 rounded p-1.5">
                        ...
                        <span>
                          {cat.label} limit exceeded! Reduce spending
                          immediately.
                        </span>
                      </div>
                    )}
      ...
      <Card className="border-muted">
        <CardContent className="pt-3 pb-3">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>
              Spending data is automatically pulled from your Transactions for
              the current month (
